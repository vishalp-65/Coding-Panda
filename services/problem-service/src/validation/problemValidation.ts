import Joi from 'joi';
import { ProblemDifficulty } from '@ai-platform/types';
    ...
    .items(
      Joi.object({
        ...
        expectedOutput: Joi.string()
          ...
          .max(10000)
          .messages({
            'string.max': 'Test case expected output cannot exceed 10000 characters'
          }),
...
export const searchProblemsSchema = Joi.object({
  ...
  difficulty: Joi.alternatives()
    .try(
      Joi.array().items(Joi.string().valid('easy', 'medium', 'hard')).unique(),
  ...
  status: Joi.string()
    .valid('solved', 'attempted', 'unsolved')
    .optional(),
