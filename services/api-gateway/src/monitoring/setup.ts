...
function setupHealthChecks(healthManager: HealthManager, logger: Logger) {
    ...
    if (process.env.REDIS_URL) {
        try {
            ...
            logger.warn('Redis health check not registered', { error: (error as Error).message });
        }
    }
    ...
    const externalServices = [
        ...
        { name: 'problem-service', url: 'http://localhost:3003/health' },
        { name: 'code-execution-service', url: 'http://localhost:8001/health' },
    ];
