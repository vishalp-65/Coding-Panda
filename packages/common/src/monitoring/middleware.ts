...
export function securityMonitoringMiddleware(config: MonitoringMiddlewareConfig) {
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        for (const { pattern, type } of suspiciousPatterns) {
            if (pattern.test(url) || pattern.test(body) || pattern.test(query) || pattern.test(headers)) {
        ...
        if (!userAgent || userAgent.length < 10) {
            ...
            config.metricsCollector.recordSecurityEvent('suspicious_user_agent', 'info');
        }
    ...
    return (req: Request, res: Response, next: NextFunction) => {
        res.on('finish', () => {
            ...
            if (res.statusCode >= 200 && res.statusCode < 300) {
                ...
                if (route.includes('/problems') && req.method === 'POST') {
                ...
                if (route.includes('/submissions') && req.method === 'POST') {
                ...
                if (route.includes('/contests') && req.method === 'POST') {
...
export function monitorDatabaseQuery<T>(
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            config.logger.error(`Database query failed: ${operation} on ${table}`, error as Error, {
                operation,
                table,
                duration,
            });
...
export function monitorQueueProcessing<T>(
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            config.logger.error(`Queue processing failed: ${queueName}`, error as Error, {
                queue: queueName,
                duration,
                status: 'failed',
            });
