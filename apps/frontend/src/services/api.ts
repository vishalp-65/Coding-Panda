...
export const problemsApi = {
  ...
  executeCode: (data: {
    ...
    problemId: string;
    testCases?: any[];
  }) => apiClient.post<any>('/code-execution/execute', data),
...
export const contestApi = {
  ...
  getLeaderboard: (id: string) => apiClient.get<any>(`/contests/${id}/leaderboard`),
  getUserSubmissions: (id: string) => apiClient.get<any[]>(`/contests/${id}/submissions`),
  getAnalytics: (id: string) => apiClient.get<any>(`/contests/${id}/analytics`),
};

// Collaboration API
export const collaborationApi = {
  createSession: (data: any) => apiClient.post<any>('/collaboration/sessions', data),
  getSession: (id: string) => apiClient.get<any>(`/collaboration/sessions/${id}`),
  joinSession: (id: string) => apiClient.post(`/collaboration/sessions/${id}/join`),
  leaveSession: (id: string) => apiClient.post(`/collaboration/sessions/${id}/leave`),
  updateSessionSettings: (id: string, settings: any) => apiClient.put(`/collaboration/sessions/${id}/settings`, settings),
  getChatHistory: (sessionId: string) => apiClient.get<any[]>(`/collaboration/sessions/${sessionId}/chat`),
};
...
export const interviewApi = {
  ...
  submitAnswer: (sessionId: string, questionId: string, answer: any) => apiClient.post(`/interviews/${sessionId}/questions/${questionId}/answer`, answer),
  getAIFeedback: (sessionId: string, questionId: string) => apiClient.get<any>(`/interviews/${sessionId}/questions/${questionId}/feedback`),
...
export const socialApi = {
  ...
  getFollowers: (userId: string) => apiClient.get<any[]>(`/social/users/${userId}/followers`),
  getFollowing: (userId: string) => apiClient.get<any[]>(`/social/users/${userId}/following`),
  getSuggestedUsers: () => apiClient.get<any[]>('/social/users/suggestions'),
  getActivityFeed: (params?: any) => apiClient.get<any[]>('/social/activity', params),
  likeActivity: (activityId: string) => apiClient.post(`/social/activity/${activityId}/like`),
  commentOnActivity: (activityId: string, comment: string) => apiClient.post(`/social/activity/${activityId}/comment`, { comment }),
};
...
export const discussionApi = {
  ...
  replyToThread: (threadId: string, content: string) => apiClient.post(`/discussions/${threadId}/replies`, { content }),
  voteOnThread: (threadId: string, voteType: 'up' | 'down') => apiClient.post(`/discussions/${threadId}/vote`, { voteType }),
  voteOnReply: (replyId: string, voteType: 'up' | 'down') => apiClient.post(`/discussions/replies/${replyId}/vote`, { voteType }),
};
